...
public class PlayerMovement_2 : MonoBehaviour
{
	...
	void FixedUpdate(){
		...
		float h = Input.GetAxisRaw ("PS4_Horizontal_2");
		float v = Input.GetAxisRaw ("PS4_Vertical_2");
		
		Move (h, v);
		Turning ();
		Animating (h, v);
		
		if((dodgeTimer >= timeBetDodge && Input.GetAxisRaw("PS4_L2_2") > 0 && dodgeInit == true) || dodgeInit == false){
	...
	void Turning(){
		...
		float hori = Input.GetAxis("PS4_RightAnalogHorizontal_2");
		float vert = Input.GetAxis("PS4_RightAnalogVertical_2");
